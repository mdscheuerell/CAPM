...
mod_res[[i]] <- MARSS(assets, inits=ini_list, model=mod_list, control=con_list)
...
## ts of alphas
alphas <- t(mod_res[[i]]$states[1:nn,])
## ts of SE{alphas}
alphas_se <- t(mod_res[[i]]$states.se[1:nn,])

## ts of betas
betas <- t(mod_res[[i]]$states[(1:nn)+nn,])
## ts of SE{betas}
betas_se <- t(mod_res[[i]]$states.se[(1:nn)+nn,])
